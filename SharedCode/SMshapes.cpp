...
namespace SMshapes {
	...
	SMmesh plane(vec3f pos, vec3f size)
	{
		SMmesh plane("Plane");
		float *cols = nullptr;

		vec3f n(0, -1.f, 0);
		plane.addPackedVert2(pos + vec3f(size.x, size.y, 0), n, vec2f(1, 1), cols);
		plane.addPackedVert2(pos + vec3f(size.x*-1.f, size.y*-1.f, 0), n, vec2f(0, 0), cols);
		plane.addPackedVert2(pos + vec3f(size.x*-1.f, size.y, 0), n, vec2f(0, 1), cols);

		plane.addPackedVert2(pos + vec3f(size.x, size.y*-1.f, 0), n, vec2f(1, 0), cols);
		plane.addPackedVert2(pos + vec3f(size.x*-1.f, size.y*-1.f, 0), n, vec2f(0, 0), cols);
		plane.addPackedVert2(pos + vec3f(size.x, size.y, 0), n, vec2f(1, 1), cols);
	...
	SMmesh sprite(vec2f pos, vec2f size)
	{
		...
		sprite.addPackedVert2(pos3 + vec3f(size.x, size.y, DEPTH2D), n, vec2f(1, 1), cols);
		sprite.addPackedVert2(pos3 + vec3f(0, 0, DEPTH2D), n, vec2f(0, 0), cols);
		sprite.addPackedVert2(pos3 + vec3f(0, size.y, DEPTH2D), n, vec2f(0, 1), cols);

		sprite.addPackedVert2(pos3 + vec3f(size.x, 0, DEPTH2D), n, vec2f(1, 0), cols);
		sprite.addPackedVert2(pos3 + vec3f(0, 0, DEPTH2D), n, vec2f(0, 0), cols);
		sprite.addPackedVert2(pos3 + vec3f(size.x, size.y, DEPTH2D), n, vec2f(1, 1), cols);
